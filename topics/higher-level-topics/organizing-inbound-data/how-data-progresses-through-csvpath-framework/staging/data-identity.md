...
Raw source files are staged by registering them with CsvPath Framework. Registration happens using the Framework's API or [the CLI](../../../../practical-stuff/the-cli.md).  When a file is registered a golden master copy is created which becomes the canonical file for that file in its raw form. In this way, CsvPath Framework becomes the source of truth and identity for data files entering the organization.&#x20;
