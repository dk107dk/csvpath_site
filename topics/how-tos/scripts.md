...
The only caveat is that you cannot run scripts after named-paths group runs unless you are using the local filesystem backend — the default. If you are storing your archive in the cloud or on an SFTP server you will need to use another method to trigger actions. [A cloud function](aws-lambda.md) would be one option. You can [use Zapier, FTTT, or another webhook savvy tool with a named-paths webhook call](webhooks.md) to trigger actions and workflows in the cloud.
